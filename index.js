...
// Функція для отримання країни за введеним значенням
async function getName(valueInput) {
  try {
    const { data } = await axios.get(`https://restcountries.com/v3.1/name/${valueInput}`);
    
    // Якщо знайдено більше 10 результатів, показуємо повідомлення
    if (data.length > 10) {
      infoList.innerHTML = '';
      PNotify.alert({
        text: "Знайдено занадто багато збігів. Будь ласка, введіть більш конкретний запит!",
    ...
    // Створюємо HTML для відображення інформації про країни
    makeHtml(data);
  } catch (error) {
    console.log(error);
    PNotify.alert({
...
// Функція для створення HTML з даних про країни
function makeHtml(countrys) {
  const markup = countrys.map(country => {
    const population = country.population;
    const languages = country.languages
...
// Дебаунс для оптимізації пошуку
const debounceSearch = _.debounce(function(value) {
  console.log("Шукаємо країну:", value);

  PNotify.alert({
    text: `Шукаємо країну: ${value}`,
    type: 'info',
    delay: 2000
  });
  ...
  axios.get(`https://restcountries.com/v3.1/name/${value}`)
    .then(function (response) {
      console.log("Дані країн отримано:", response.data);
